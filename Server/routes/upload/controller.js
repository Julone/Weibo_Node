...
exports.img = function (req, res) {
  ...
  let uploadFileList = [];
  form.uploadDir = easyPath(APP_ROOT,'uploads/tmp');
  ...
  form.parse(req, function (err, fields, files) {
    ...
    async.map(uploadFileList,function (el, cb) {
      let file_name = md5((Date.now()) + el.size + el.name) + extname(el.name);
      let distPath = easyPath(APP_ROOT,'uploads',file_name);
