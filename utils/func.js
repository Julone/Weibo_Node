...
export function postDataCheck(res,args){
    // postDataCheck(res, { usermail: { value: usermail, exp: 'mail' }, code: code });
    return new Promise((reject,resolve)=>{
        console.log(args);
        if( _.isObject(args)){
            async.forEachOf(args,(el,key,cb)=>{
                let item = el;
                let RE = /./;
                let code = "";
                if(_.isObject(item)){
                    ...
                    if( !isNullVal(item.exp)){
                        var REarr = require('./RegExpress');
                        switch(item.exp){
                            case 'mail': { 
                                RE = REarr.email;
                                code = 104
